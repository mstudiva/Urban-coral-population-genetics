---
title: "*Pseudodiploria clivosa* 2bRAD"
...
```{bash, bowtie align 2}
cd ~/urban/pcli/filteredReads
conda activate 2bRAD
HOSTGENOME=~/db/pcliGenome/PclivosaGenome
mkdir junk
...
```{bash, ANGSD}
export FILTERS="-uniqueOnly 1 -remove_bads 1 -minMapQ 20 -maxDepth 930 -minInd 46"
export TODO="-doQsDist 1 -doDepth 1 -doCounts 1 -dumpCounts 2"

echo '#!/bin/bash' >pcliDD.sh
echo angsd -b bamsClones -GL 1 $FILTERS $TODO -P 1 -out dd -nThreads 20 >>pcliDD.sh

sbatch --mem=200GB -o pcliDD.o%j -e pcliDD.e%j --mail-user=studivanms@gmail.com --mail-type=ALL --partition=shortq7 pcliDD.sh
```
...
```{bash, ANGSD 2}
FILTERS="-uniqueOnly 1 -remove_bads 1 -minMapQ 20 -minQ 30 -dosnpstat 1 -doHWE 1 -hwe_pval 1e-5 -sb_pval 1e-5 -hetbias_pval 1e-5 -skipTriallelic 1 -minInd 74 -snp_pval 1e-6 -minMaf 0.05"
...
cat bamsClones | grep -v 'PCLI_urban_011_2.trim.un.bt2.bam\|PCLI_urban_011_3.trim.un.bt2.bam\|PCLI_urban_011.trim.un.bt2.bam\|PCLI_urban_037.trim.un.bt2.bam\|PCLI_urban_078_2.trim.un.bt2.bam\|PCLI_urban_078_3.trim.un.bt2.bam\|PCLI_urban_092_2.trim.un.bt2.bam\|PCLI_urban_092_3.trim.un.bt2.bam\|PCLI_urban_120.trim.un.bt2.bam\|PCLI_urban_126.trim.un.bt2.bam\|PCLI_urban_181.trim.un.bt2.bam\|PCLI_urban_188.trim.un.bt2.bam\|PCLI_urban_230_3.trim.un.bt2.bam' >bamsNoClones

module load angsd-0.933-gcc-9.2.0-65d64pp
FILTERS="-uniqueOnly 1 -remove_bads 1 -minMapQ 20 -minQ 30 -dosnpstat 1 -doHWE 1 -hwe_pval 1e-5 -sb_pval 1e-5 -hetbias_pval 1e-5 -skipTriallelic 1 -minInd 64 -snp_pval 1e-6 -minMaf 0.05"
TODO="-doMajorMinor 1 -doMaf 1 -doCounts 1 -makeMatrix 1 -doIBS 1 -doCov 1 -doGeno 8 -doBcf 1 -doPost 1 -doGlf 2"

echo '#!/bin/bash' > pcliNoClones.sh
echo angsd -b bamsNoClones -GL 1 $FILTERS $TODO -P 1 -out pcliNoClones >> pcliNoClones.sh
...
```{bash, SNPs? 1}
grep "filtering:" pcliNoClones.e*
```
Number of sites retained after filtering: 15888 
...
cp ../ANGSD/pcliNoClones.beagle.gz .

conda activate pcangsd

echo '#!/bin/bash' > pcangsd
echo 'pcangsd -b pcliNoClones.beagle.gz -o pcliPcangsd --admix --inbreed-samples --pcadapt --selection' >> pcangsd
...
ngsAdmixLauncher.py -f pcliNoClones.beagle.gz --maxK 17 -r 50 -n pcli --launcher -e studivanms@gmail.com
sbatch --mem=200GB pcliNgsAdmix.slurm
```
...
```{bash, clumpak}
>pcliNgsAdmixLogfile
for log in pcli*.log; do
grep -Po 'like=\K[^ ]+' $log >> pcliNgsAdmixLogfile;
done
```
...
logs <- as.data.frame(read.table("pcliNgsAdmixLogfile"))

# output is organized with 10, 11 preceding 1, 2, 3 etc.
logs$K <- c(rep("10", 50), rep("11", 50), rep("12", 50), rep("13", 50), rep("14", 50), rep("15", 50), rep("16", 50), rep("17", 50), rep("1", 50), rep("2", 50), rep("3", 50), rep("4", 50), rep("5", 50), rep("6", 50), rep("7", 50), rep("8", 50), rep("9", 50))
write.table(logs[, c(2, 1)], "pcliNgsAdmixLogfile_formatted", row.names = F, col.names = F, quote = F)
quit()
...
```{bash, check rows}
cat pcliNgsAdmixLogfile_formatted | wc -l
```
850, so yes (K=17 * 50 simulations each)

<br>

Make copies of .qopt files to run structure selector on (.Q files)
```{bash, qloop}
for file in pcli*.qopt; do
...
zip -r pcliQ.zip pcliQ
```
scp .zip, formatted logfile, and pcliNoClones* to local machine
...
```
![](../../figures/pcli/DeltaK_and_MeanLnP_plot.png)
...
K=1
![](../../figures/pcli/K=1.png)
K=2
![](../../figures/pcli/K=2.png)
K=3
![](../../figures/pcli/K=3.png)

K=4
![](../../figures/pcli/K=4.png)
