...
popfile <- read.table(file = "../../data/dlab/ngsAdmix/dlabPopfile.txt", header = FALSE, col.names=c("sample", "site"), stringsAsFactors=FALSE)
...
write.table(popfile_sorted, "../../data/dlab/ngsAdmix/dlabPopfile_sorted.txt",
            quote=FALSE, row.names=FALSE, col.names=FALSE, sep="\t")

# Unzips, loops through, and reorders each .Q file based on popfile_sorted
unzip("../../data/dlab/ngsAdmix/dlabQ.zip", exdir = "../../data/dlab/ngsAdmix/")

q_dir  <- "../../data/dlab/ngsAdmix/dlabQ"              # where .Q files now live
out_dir <- "../../data/dlab/ngsAdmix/dlabQ_sorted"   # new folder for reordered files
...
# Cleanup
unlink(q_dir, recursive = TRUE) # Delete the uncompressed dlabQ directory 

# Zip the dlabQ_sorted directory
out_dir_norm <- normalizePath(out_dir)
parent_dir   <- dirname(out_dir_norm)
folder_name  <- basename(out_dir_norm)

zipfile_path <- file.path(parent_dir, "dlabQ_sorted.zip")
cmd <- sprintf(
  'cd "%s" && zip -r "%s" "%s"',
  parent_dir,
...
} # Delete the uncompressed dlabQ_sorted directory 
```
