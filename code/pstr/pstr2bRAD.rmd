---
title: "*Pseudodiploria strigosa* 2bRAD"
...
knit: (function(input, ...) {
    rmarkdown::render(
      input,
      output_file = '../pstr strigosa/index.html',
      envir = globalenv()
    )
  })
---
...
```{bash, bowtie align 2}
cd ~/urban/pstr strigosa/filteredReads
conda activate 2bRAD
HOSTGENOME=~/db/pstr strigosaGenome/pstr strigosaeolataGenome
mkdir junk
...
echo '#!/bin/bash' >pstr strigosaDD.sh
echo angsd -b bamsClones -GL 1 $FILTERS $TODO -P 1 -out dd -nThreads 20 >>pstr strigosaDD.sh

sbatch --mem=200GB -o pstr strigosaDD.o%j -e pstr strigosaDD.e%j --mail-user=studivanms@gmail.com --mail-type=ALL --partition=shortq7 pstr strigosaDD.sh
```
...
echo '#!/bin/bash' > pstr strigosaClones.sh
echo angsd -b bamsClones -GL 1 $FILTERS $TODO -P 1 -out pstr strigosaClones >>pstr strigosaClones.sh
...
cloneBams = read.csv("../../data/pstr strigosa/pstr strigosaMetadata.csv") # list of bam files

cloneMa = as.matrix(read.table("../../data/pstr strigosa/ANGSD/clones/pstr strigosaClones.ibsMat")) # reads in IBS matrix produced by ANGSD 
...
ggsave("../../figures/pstr strigosa/cloneDend.pdf", plot = cloneDend, height = 8, width = 24, units = "in", dpi = 300)
ggsave("../../figures/pstr strigosa/cloneDend.png", plot = cloneDend, height = 8, width = 24, units = "in", dpi = 300)
```
![](../../figures/pstr strigosa/cloneDend.png)
<br>
...
mkdir clones
mv pstr strigosaClones* clones

ll *.bam > bamsNoClones

cat bamsClones | grep -v 'pstr strigosa_urban_359.trim.un.bt2.bam\|pstr strigosa_nmfs_035.trim.un.bt2.bam\|pstr strigosa_nmfs_069.trim.un.bt2.bam\|pstr strigosa_nmfs_041.trim.un.bt2.bam\|pstr strigosa_urban_080_2.trim.un.bt2.bam\|pstr strigosa_urban_080_3.trim.un.bt2.bam\|pstr strigosa_urban_289_2.trim.un.bt2.bam\|pstr strigosa_urban_081.trim.un.bt2.bam\|pstr strigosa_urban_415.trim.un.bt2.bam' >bamsNoClones
...
echo '#!/bin/bash' > pstr strigosaNoClones.sh
echo angsd -b bamsNoClones -GL 1 $FILTERS $TODO -P 1 -out pstr strigosaNoClones >> pstr strigosaNoClones.sh
...
```{bash, SNPs? 1}
grep "filtering:" pstr strigosaNoClones.e*
```
...
cp ../ANGSD/pstr strigosaNoClones.beagle.gz .

conda activate pcangsd

echo '#!/bin/bash' > pcangsd.sh
echo 'pcangsd -b pstr strigosaNoClones.beagle.gz -o pstr strigosaPcangsd --admix --inbreed-samples --pcadapt --selection' >> pcangsd
...
ngsAdmixLauncher.py -f pstr strigosaNoClones.beagle.gz --maxK 16 -r 50 -n pstr strigosa --launcher -e studivanms@gmail.com
sbatch --mem=200GB pstr strigosaNgsAdmix.slurm
```
...
```{bash, clumpak}
>pstr strigosaNgsAdmixLogfile
for log in pstr strigosa*.log; do
grep -Po 'like=\K[^ ]+' $log >> pstr strigosaNgsAdmixLogfile;
done
```
...
logs <- as.data.frame(read.table("pstr strigosaNgsAdmixLogfile"))
...
write.table(logs[, c(2, 1)], "pstr strigosaNgsAdmixLogfile_formatted", row.names = F, col.names = F, quote = F)
quit()
...
```{bash, check rows}
cat pstr strigosaNgsAdmixLogfile_formatted | wc -l
```
...
```{bash, qloop}
for file in pstr strigosa*.qopt; do
filename=$(basename -- "$file" .qopt);
cp "$file" "$filename".Q;
done

mkdir pstr strigosaQ
mv pstr strigosa*Q pstr strigosaQ

zip -r pstr strigosaQ.zip pstr strigosaQ
```
scp .zip, formatted logfile, and pstr strigosaNoClones* to local machine
...
# -------------------------
data_file <- "../../data/pstr strigosa/ngsAdmix/pstr strigosaNgsAdmixLogfile_formatted"  # replace with your file
...
# -------------------------
output_file <- "../../figures/pstr strigosa/DeltaK_and_MeanLnP_plot.png"
...
```
![](../../figures/pstr strigosa/DeltaK_and_MeanLnP_plot.png)
...
K=1
![](../../figures/pstr strigosa/K=1.png)
K=2
![](../../figures/pstr strigosa/K=2.png)
K=3
![](../../figures/pstr strigosa/K=3.png)

K=4
![](../../figures/pstr strigosa/K=4.png)
