...
# 6.If coral detected in one technical replicate, remove the sample
    ...
    ReRun_Dlab <- Dlab[which(Dlab$A.reps==1 | Dlab$C.reps==1 |Dlab$D.reps==1), ] # 1 sample violates, but not necessary to rerun
...
# 7. High ST    
    ...
    StDe1.5_Dlab <- Dlab[which(Dlab$D.CT.sd>1.5 | Dlab$A.CT.sd>1.5 | Dlab$C.CT.sd>1.5), ] # no samples violate
...
# 8.Suspiciously late coral amplification                                 NOT NEEDED
  ...
  Late_Dlab<-Dlab[which(Dlab$D.CT.mean>25 & Dlab$A.CT.mean>25 & Dlab$C.CT.mean>25), ]
